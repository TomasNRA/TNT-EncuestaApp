...
import com.example.encuestaapp.databinding.FragmentEncuestaPastelDePapaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaPastelDePapaFragment : Fragment() {
    private var _binding: FragmentEncuestaPastelDePapaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Pastel de papa"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaPastelDePapaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setPastelDePapa(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaPastelDePapaFragment_to_encuestaPucheroFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaPastelDePapaFragment_to_homeFragment)
        }
    }
