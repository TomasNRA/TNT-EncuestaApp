...
import com.example.encuestaapp.databinding.FragmentEncuestaBebidaBlancaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaBebidaBlancaFragment : Fragment() {
    private var _binding: FragmentEncuestaBebidaBlancaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Bebida blanca"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaBebidaBlancaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setBebidaBlanca(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaBebidaBlancaFragment_to_confirmacionFragment2)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaBebidaBlancaFragment_to_homeFragment)
        }
    }
