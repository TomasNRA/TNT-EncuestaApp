...
import com.example.encuestaapp.databinding.FragmentEncuestaHuevoDeGallinaCrudoHervidoPocheBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaHuevoDeGallinaCrudoHervidoPocheFragment : Fragment() {
    private var _binding: FragmentEncuestaHuevoDeGallinaCrudoHervidoPocheBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Huevo de gallina crudo hervido poche"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaHuevoDeGallinaCrudoHervidoPocheBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setHuevoDeGallinaCrudoHervidoPoche(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaHuevoDeGallinaCrudoHervidoPocheFragment_to_encuestaHuevoDeGallinaFritoFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaHuevoDeGallinaCrudoHervidoPocheFragment_to_homeFragment)
        }
    }
