...
import com.example.encuestaapp.databinding.FragmentEncuestaLicorBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaLicorFragment : Fragment() {
    private var _binding: FragmentEncuestaLicorBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Licor"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaLicorBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setLicor(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaLicorFragment_to_encuestaBebidaBlancaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaLicorFragment_to_homeFragment)
        }
    }
