...
import com.example.encuestaapp.databinding.FragmentEncuestaDuraznoBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaDuraznoFragment : Fragment() {
    private var _binding: FragmentEncuestaDuraznoBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Durazno"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaDuraznoBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setDurazno(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaDuraznoFragment_to_encuestaManzanaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaDuraznoFragment_to_homeFragment)
        }
    }
