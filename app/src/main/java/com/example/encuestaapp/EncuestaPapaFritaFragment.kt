...
import com.example.encuestaapp.databinding.FragmentEncuestaPapaFritaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaPapaFritaFragment : Fragment() {
    private var _binding: FragmentEncuestaPapaFritaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Papa frita"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaPapaFritaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setPapafrita(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaPapaFritaFragment_to_encuestaPizzaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaPapaFritaFragment_to_homeFragment)
        }
    }
