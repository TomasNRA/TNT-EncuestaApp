...
import com.example.encuestaapp.databinding.FragmentEncuestaAguaSaborizadaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaAguaSaborizadaFragment : Fragment() {
    private var _binding: FragmentEncuestaAguaSaborizadaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Agua saborizada"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaAguaSaborizadaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setAguaSaborizada(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaAguaSaborizadaFragment_to_encuestaBebidaDeportivaOEnergizanteFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaAguaSaborizadaFragment_to_homeFragment)
        }
    }
