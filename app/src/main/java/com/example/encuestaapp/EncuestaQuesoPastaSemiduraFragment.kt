...
import com.example.encuestaapp.databinding.FragmentEncuestaQuesoPastaSemiduraBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaQuesoPastaSemiduraFragment : Fragment() {
    private var _binding: FragmentEncuestaQuesoPastaSemiduraBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Queso de pasta semidura"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaQuesoPastaSemiduraBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setQuesoPastaSemidura(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaQuesoPastSemiaduraFragment_to_encuestaMantecaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaQuesoPastaSemiduraFragment_to_homeFragment)
        }
    }
