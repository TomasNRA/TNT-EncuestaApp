...
import com.example.encuestaapp.databinding.FragmentEncuestaEmpanadaDeCarneFritaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaEmpanadaDeCarneFritaFragment : Fragment() {
    private var _binding: FragmentEncuestaEmpanadaDeCarneFritaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Empanada de carne frita"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaEmpanadaDeCarneFritaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setEmpanadaDeCarneFrita(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaEmpanadaDeCarneFritaFragment_to_encuestaEmpanadaDeCarneFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaEmpanadaDeCarneFritaFragment_to_homeFragment)
        }
    }
