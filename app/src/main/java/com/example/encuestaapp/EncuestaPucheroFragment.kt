...
import com.example.encuestaapp.databinding.FragmentEncuestaPucheroBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaPucheroFragment : Fragment() {
    private var _binding: FragmentEncuestaPucheroBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Puchero"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaPucheroBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setPuchero(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaPucheroFragment_to_encuestaBananaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaPucheroFragment_to_homeFragment)
        }
    }
