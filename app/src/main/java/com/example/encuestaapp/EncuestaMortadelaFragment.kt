...
import com.example.encuestaapp.databinding.FragmentEncuestaMortadelaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaMortadelaFragment : Fragment() {
    private var _binding: FragmentEncuestaMortadelaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Mortadela"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaMortadelaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setMortadela(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaMortadelaFragment_to_encuestaPapaFritaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaMortadelaFragment_to_homeFragment)
        }
    }
