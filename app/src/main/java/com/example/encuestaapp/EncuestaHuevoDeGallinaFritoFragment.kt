...
import com.example.encuestaapp.databinding.FragmentEncuestaHuevoDeGallinaFritoBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaHuevoDeGallinaFritoFragment : Fragment() {
    private var _binding: FragmentEncuestaHuevoDeGallinaFritoBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Huevo de gallina frito"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaHuevoDeGallinaFritoBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setHuevoDeGallinaFrito(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaHuevoDeGallinaFritoFragment_to_encuestaCarneVacunaMagraFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaHuevoDeGallinaFritoFragment_to_homeFragment)
        }
    }
