...
import com.example.encuestaapp.databinding.FragmentEncuestaGaseosaClasicaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaGaseosaClasicaFragment : Fragment() {
    private var _binding: FragmentEncuestaGaseosaClasicaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Gaseosa clasica"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaGaseosaClasicaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setGaseosaClasica(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaGaseosaClasicaFragment_to_encuestaVinoFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaGaseosaClasicaFragment_to_homeFragment)
        }
    }
