...
import com.example.encuestaapp.databinding.FragmentEncuestaLecheFluidaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat

class EncuestaLecheFluidaFragment : Fragment() {
    private var _binding: FragmentEncuestaLecheFluidaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Leche fluida"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaLecheFluidaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setLecheFluida(alimenmto)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaLecheFluidaFragment_to_encuestaQuesoPastaDuraFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaLecheFluidaFragment_to_homeFragment)
        }
    }
