...
import com.example.encuestaapp.databinding.FragmentEncuestaManzanaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaManzanaFragment : Fragment() {
    private var _binding: FragmentEncuestaManzanaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Manzana"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaManzanaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setManzana(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaManzanaFragment_to_encuestaAguaSaborizadaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaManzanaFragment_to_homeFragment)
        }
    }
