...
import com.example.encuestaapp.databinding.FragmentEncuestaBananaBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaBananaFragment : Fragment() {
    private var _binding: FragmentEncuestaBananaBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Banana"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaBananaBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setBanana(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaBananaFragment_to_encuestaDuraznoFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaBananaFragment_to_homeFragment)
        }
    }
