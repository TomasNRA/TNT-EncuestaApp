...
import com.example.encuestaapp.databinding.FragmentEncuestaCarneVacunaCortesGrasosBinding
import com.example.encuestaapp.db.entitis.AlimentoEncuesta
import java.text.DecimalFormat


class EncuestaCarneVacunaCortesGrasosFragment : Fragment() {
    private var _binding: FragmentEncuestaCarneVacunaCortesGrasosBinding? = null
    private val binding get() = _binding!!

    private val encuestaViewModel : EncuestaViewModel by activityViewModels()

    private var alimento:String = "Carne vacuna cortes grasos"
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEncuestaCarneVacunaCortesGrasosBinding.inflate(layoutInflater, container, false)
        initListeners()
        return binding.root
    }
    ...
    private fun initListeners() {
        ...
        binding.btnContinuar.setOnClickListener {
            if(::porcion.isInitialized && ::frecuencia.isInitialized && cantidad != 0){
                ...
                encuestaViewModel.setCarneVacunaCortesGrasos(alimento)
                //encuestaViewModel.insert()
                findNavController().navigate(R.id.action_encuestaCarneVacunaCortesGrasosFragment_to_encuestaCarnePicadaFragment)
            }else {
                Toast.makeText(
                    this.context,
        ...
        binding.btnCancelar.setOnClickListener {
            findNavController().navigate(R.id.action_encuestaCarneVacunaCortesGrasosFragment_to_homeFragment)
        }
    }
